> **АРХИВ — ВЫПОЛНЕН 2026-02-08.**

# Замена Popup Maker на кастомный попап — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Заменить плагин Popup Maker (~107 КБ фронтенд-ресурсов) на лёгкий кастомный модальный попап (<2 КБ), сохранив полную функциональность формы Битрикс24.

**Architecture:** Попап реализуется как inline HTML в `wp_footer` через хук в `bitrix24-lead-form.php`. Модалка открывается по клику на `a[href="#b24-modal"]` (замена `#popmake-8502`). CSS/JS минимальный, встроен в тот же хук. AJAX-обработчик `b24_submit_lead` остаётся без изменений.

**Tech Stack:** PHP (WordPress hooks), vanilla JS, CSS. Без jQuery-зависимости.

---

## Текущее состояние

- **Popup Maker** создаёт 1 попап (ID 8502) с шорткодом `[bitrix24_form]`
- Кнопки-триггеры: `<a href="#popmake-8502">` — 4 места в 3 файлах:
  - `functions-new.php:82` — кнопка «Получить КП» на стандартных карточках товаров
  - `single-product-redesign.php:1094,1109,1223` — кнопки в редизайне страницы товара
  - `page-vsuzi-hub.php:878,1170` — кнопки на странице Vsuzi Hub
- Форма (`bitrix24-lead-form.php`) уже содержит весь HTML, CSS, JS формы — Popup Maker лишь обёртка
- `bitrix24-lead-form.php` стилизует pum-контейнер через `!important` (строки 106–144) — будет удалено
- В `functions-new.php:610-611` уже хак для удаления deps Popup Maker block-library-style

## Файлы которые меняем

| Файл | Действие |
|------|----------|
| `bitrix24-lead-form.php` | Переписываем: убираем shortcode, добавляем wp_footer хук с модалкой |
| `functions-new.php` | Меняем `#popmake-8502` → `#b24-modal`, убираем PM-хаки |
| `single-product-redesign.php` | Меняем `#popmake-8502` → `#b24-modal` |
| `page-vsuzi-hub.php` | Меняем `#popmake-8502` → `#b24-modal` |

---

### Task 1: Переписать bitrix24-lead-form.php — модалка без Popup Maker

**Files:**
- Modify: `bitrix24-lead-form.php`

**Step 1: Заменить содержимое bitrix24-lead-form.php**

Убираем shortcode `[bitrix24_form]` и `b24_product_capture_js`. Вместо них — `wp_footer` хук, выводящий полную модалку с оверлеем. AJAX-обработчик `b24_submit_lead` остаётся без изменений.

Новый `bitrix24-lead-form.php`:
...
- Убран `add_shortcode('bitrix24_form', ...)` — больше не нужен
- Убран `b24_product_capture_js()` — логика захвата товара встроена в открытие модалки
- Убраны все стили `#pum-8502` / `.pum-container` — заменены на `.b24-overlay` / `.b24-modal`
- Модалка выводится через `wp_footer` хук (не через shortcode в Popup Maker)
- Закрытие: крестик, клик по оверлею, Escape
- Автозакрытие через 2 секунды после успешной отправки (как раньше)
- Без jQuery — `PUM.close()` заменён на `closeModal()`
- Добавлен захват h1 для страниц товара (на всякий случай, если нет card-контейнера)

**Step 2: Деплой и проверка рендеринга**

```bash
scp bitrix24-lead-form.php root@85.198.96.28:/var/www/tdpuls.com/public/wp-content/themes/flatsome/bitrix24-lead-form.php
```

Проверить: открыть tdpuls.com → View Source → найти `id="b24-modal"` в конце body.

---

### Task 2: Обновить триггер-ссылки — заменить #popmake-8502 на #b24-modal

**Files:**
- Modify: `functions-new.php:82`
- Modify: `single-product-redesign.php:1094,1109,1223`
- Modify: `page-vsuzi-hub.php:878,1170`

**Step 1: Заменить все `#popmake-8502` на `#b24-modal`**

В каждом файле — find & replace:
- `#popmake-8502` → `#b24-modal`

Точные замены:

`functions-new.php:82`:
```php
# Было:
<a href="#popmake-8502" class="button button--big">
# Стало:
<a href="#b24-modal" class="button button--big">
```

`single-product-redesign.php:1094`:
```php
# Было:
<a href="#popmake-8502" class="tdp-leasing">
# Стало:
<a href="#b24-modal" class="tdp-leasing">
```

`single-product-redesign.php:1109`:
```php
# Было:
<a href="#popmake-8502" class="tdp-btn-primary">
# Стало:
<a href="#b24-modal" class="tdp-btn-primary">
```

`single-product-redesign.php:1223`:
```php
# Было:
<a href="#popmake-8502" class="tdp-btn-white">
# Стало:
<a href="#b24-modal" class="tdp-btn-white">
```

`page-vsuzi-hub.php:878`:
```php
# Было:
<a href="#popmake-8502" class="tdp-btn tdp-btn-primary">
# Стало:
<a href="#b24-modal" class="tdp-btn tdp-btn-primary">
```

`page-vsuzi-hub.php:1170`:
```php
# Было:
<a href="#popmake-8502" class="tdp-btn tdp-btn-primary">
# Стало:
...
Также в `single-product-redesign.php:1382` — убрать блок JS `document.querySelectorAll('a[href="#popmake-8502"]')...` (эта логика теперь в `bitrix24-lead-form.php`).

**Step 2: Деплой всех трёх файлов**

```bash
scp functions-new.php root@85.198.96.28:/var/www/tdpuls.com/public/wp-content/themes/flatsome/functions.php
scp single-product-redesign.php root@85.198.96.28:/var/www/tdpuls.com/public/wp-content/themes/flatsome/single-product-redesign.php
scp page-vsuzi-hub.php root@85.198.96.28:/var/www/tdpuls.com/public/wp-content/themes/flatsome/page-vsuzi-hub.php
```

---

### Task 3: Убрать хаки Popup Maker из functions-new.php

**Files:**
- Modify: `functions-new.php:607-617`

**Step 1: Удалить блок очистки PM deps**

Убрать строки 607-617 (хак `popup-maker-block-library-style` deps + dequeue `wp-block-editor`/`wp-components`/`wp-preferences`):

```php
# Удалить:
    // 1. Popup Maker block-library-style тянет wp-block-editor → wp-components + wp-preferences (≈220 KB)
    //    Убираем зависимости у PM, чтобы block editor CSS не подгружался
    global $wp_styles;
    if (isset($wp_styles->registered['popup-maker-block-library-style'])) {
        $wp_styles->registered['popup-maker-block-library-style']->deps = array();
    }

    // Деквеим block editor CSS (не деригистрируем — безопаснее)
    wp_dequeue_style('wp-block-editor');
    wp_dequeue_style('wp-components');
    wp_dequeue_style('wp-preferences');
```

**Примечание:** `wp-block-editor`, `wp-components`, `wp-preferences` — эти CSS загружались ТОЛЬКО из-за Popup Maker. Без PM их никто не подтягивает, так что dequeue тоже не нужен.
...
### Task 4: Деактивировать и удалить Popup Maker

**Step 1: Деактивировать плагин**

```bash
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp plugin deactivate popup-maker --allow-root"
```

**Step 2: Проверить сайт**

Открыть в браузере:
- `https://tdpuls.com/` — кнопка «Получить КП» должна открывать модалку
- `https://tdpuls.com/product/любой-товар/` — проверить модалку
- `https://tdpuls.com/product/любой-товар/?design=new` — проверить модалку в редизайне
- Отправить тестовую заявку → проверить лид в Битрикс24

**Step 3: Удалить плагин (после проверки)**

```bash
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp plugin delete popup-maker --allow-root"
```
...
### Task 5: Проверить в БД — удалить пост popup и meta

**Step 1: Удалить пост попапа из БД**

```bash
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp post delete 8502 --force --allow-root"
```

**Step 2: Очистить transients и опции Popup Maker**

```bash
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp option list --search='*pum*' --allow-root --format=table"
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp option list --search='*popup*' --allow-root --format=table"
```

Удалить найденные опции:
```bash
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp option list --search='*pum*' --field=option_name --allow-root | xargs -I{} wp option delete {} --allow-root"
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp option list --search='*popup*' --field=option_name --allow-root | xargs -I{} wp option delete {} --allow-root"
```

**Step 3: Удалить сгенерированные файлы Popup Maker**

```bash
ssh root@85.198.96.28 "rm -rf /var/www/tdpuls.com/public/wp-content/uploads/pum/"
```
...
**Step 1: Проверить ресурсы фронтенда**

```bash
curl -s https://tdpuls.com/ | grep -c 'popup-maker\|pum-'
# Ожидание: 0
curl -s https://tdpuls.com/ | grep -c 'b24-modal'
# Ожидание: >= 1
```

**Step 2: Проверить счётчики ресурсов**

```bash
ssh root@85.198.96.28 "cd /var/www/tdpuls.com/public && wp plugin list --status=active --allow-root --format=count"
# Ожидание: 10 (было 11)
```

**Step 3: Проверить отправку формы через curl**

```bash
# Получить nonce через страницу
curl -s https://tdpuls.com/ | grep -o 'b24_lead_form.*"' | head -1
```

Ручная проверка в браузере: открыть модалку, заполнить, отправить, проверить лид в Б24.
...
| Метрика | До | После |
|---------|-----|-------|
| Фронтенд JS (Popup Maker) | 62 КБ + 39 КБ vendor | 0 |
| Фронтенд CSS (Popup Maker) | 7.4 КБ + 7.5 КБ themes | 0 |
| Кастомный JS/CSS (inline) | — | ~1.5 КБ |
| HTTP-запросов (Popup Maker) | 3-4 | 0 |
| Активных плагинов | 11 | **10** |
| jQuery зависимость в форме | Да (PUM.close) | **Нет** |
| Google Fonts (Montserrat, PM themes) | Загружается | **Нет** |
